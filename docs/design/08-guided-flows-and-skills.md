...
wire = [ (단계 armed면) step.skillIds의 스킬 body들(세션형은 대화당 1회;
         ...
         (기반 단계면) preflight 컨텍스트(코드베이스 경로 / RAG 발췌 / 지식 본문),
         prompt ].join("\n\n")
```
프리필 턴만 예외로 wire = `prefillInstruction(...)`이고, **스텝 커서/스킬 주입 이력/세션/영속화를 건드리지
않는다**(격리 실행). 기반 단계(rag/knowledge)의 preflight는 wire 조립 **전에** 백엔드를 조회하며,
미설정·0건·실패면 에이전트 턴 없이 건너뛴다(D44).
...
카테고리 진입
  └─ optionsFor(category, settings) 비었나?
       ...
       │                       └─ (plan 기본, 기반 3단계 — D44)
       │                          코드베이스 분석(codebase, extraDirs=--add-dir)
...
- **스킬 리소스 폴더(D45)**: 스킬 카드의 "리소스 폴더"로 참고 파일/스크립트 폴더(`SkillDef.dir`)를
  ...
  `RunArgs.extraDirs`(claude `--add-dir`)로 매 턴 접근 허용된다. 폴더 존재 검사는 없다(없으면
  에이전트가 읽기 실패로 표면화 — graceful degrade).
...
- 단계 규칙:
  ...
  - **기반 3단계(D44)는 고정(pinned)**: 편집기에서 삭제·이동·kind 변경 불가(지시문·스킬·file은 편집
    가능). `plan`은 항상 강제(`coerceSteps`가 프리펜드·보충), 그 외 카테고리는 "기반 3단계 사용" 토글로
    트리오를 삽입/제거(저장 배열의 기반 kind 존재 = 활성 플래그). rag/knowledge 단계는 지시문 아래에
...
- **wire 조립**: `send()` — `[step.skillIds의 스킬 body들(armed, 세션형 dedupe; dir 안내 부착) →
  step.instruction(단계 armed) → preflight 컨텍스트(기반 단계) → prompt]`. 프리필은
  `prefillInstruction`. 단계/스킬은 `runtimeWorkflowFor(category, settings)`/`resolveSkills(settings)`로
  해석해 **마운트 시 고정**. 매 턴 `RunArgs.extraDirs = [codebasePath, ...주입된 스킬 dir들]`.
- **preflight**: `stepPreflight(step)` — wire 조립 전 비동기 훅. `codebase`=경로 문구(동기),
  `rag`=`rag_search`(캔버스 '검색 결과' 탭 + 발췌), `knowledge`=`list_knowledge`(16KB 상한). 건너뜀 시
  system 안내 + 커서 전진 + 다음 생성형 단계로 재진입(실제 사용자 프롬프트는 이월). preflight 중 Stop은
  `preflightAbortRef`로 취소 폴백.
