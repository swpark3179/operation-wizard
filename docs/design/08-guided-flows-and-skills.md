...
```
wire = [ (대화의 첫 실제 작업 턴이면) 프롬프트 최적화 내장 스킬(promptCraft.ts — 레지스트리 밖
           하드코딩, 단계와 무관하게 1회; ```prompt 펜스 → 캔버스 '프롬프트' 탭 — D65),
...
카테고리 진입
  └─ optionsFor(category, settings) 비었나?
       ├─ 아니오 → onClarify([userRequest(text, 필수 — D65), codebasePath(folder, 필수)*, ...옵션])
       │            [즉시 폼 · 채팅 차단]
       │            └─ seedPrompt 있으면 → userRequest를 seed 원문으로 즉시 채움(클라이언트) +
       │               프리필 턴(숨김, folder·noPrefill 제외) → onPrefill(seed 병합 폼 자동채움)
       │            └─ 사용자 폼 제출 → codebasePath 상태 분리 + answerSubmission(requirement 동반)
       │               [차단 해제 · 탭 소멸]
       │                  └─ 첫 작업 턴: 프롬프트 최적화 내장 스킬(D65) + WF[0].skills +
       │                     WF[0].instruction + preflight + 답변(요구사항 포함 — 원요청 접미사 대체)
       │                     └─ end에서 ```prompt 펜스 파싱 → 캔버스 '프롬프트' 탭(채팅은 안내문 치환)
       │                       └─ (plan 기본, 기반 3단계 — D44)
...
- 항목은 `ClarifyQuestion`(`single`/`multi`/`text`/`folder`) 스키마 그대로. `single/multi`는 `options`
  ...
  폴더 픽커 버튼 + 경로 칩)이 자동 처리(D35/D45). **folder·`noPrefill` 질문은 프리필에서 제외**된다.
- `optionsFor(category, settings)`가 **모든 카테고리 폼 맨 앞에 필수 `userRequest` text 질문**
  (`REQUIREMENT_QUESTION`, `noPrefill` — D65)을, 기반 단계 활성 카테고리는 그 다음에 필수
  `codebasePath` folder 질문을 프리펜드한다(D45). 카탈로그가 비어도 요구사항 질문 때문에 옵션 단계는
  항상 존재한다(seed-first 경로는 폴백으로만 남음).
...
- `src/lib/clarify.ts`의 **fenced-block 패턴을 복제**한다(참조 구현: `parseClarify`/`stripClarifyBlock`
  = 에이전트가 질문을 생성, `parsePrefill` = 에이전트가 고정 항목을 채움; 실전 예:
  `src/lib/promptCraft.ts`의 ` ```prompt ` 계약 — 파싱 후 캔버스 탭 리프트 + 채팅 content 치환, D65).
...
- **wire 조립**: `send()` — workdir 확정(`ensure_project`)을 먼저 수행한 뒤(D52)
  `[프롬프트 최적화 내장 스킬(첫 실제 작업 턴 1회 — `promptSkillPendingRef`, 되감기는 unwindSkills
  동승, D65) → step.skillIds의 스킬 body들(armed, 세션형 dedupe; dir 안내 부착) →
  step.instruction(단계 armed) →
