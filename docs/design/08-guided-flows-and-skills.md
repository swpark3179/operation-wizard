...
- **wire 조립**: `send()` — workdir 확정(`ensure_project`)을 먼저 수행한 뒤(D52)
  ...
  매 턴 `RunArgs.extraDirs = [codebasePath, ...주입된 스킬 dir들]`(claude `--add-dir`, gemini/aipro
  `--include-directories`). `send()`는 처리 여부를 boolean으로 반환하고 폼 제출/자동전진 nonce는 성공
  시에만 소비된다(D55).
- **preflight**: `stepPreflight(step)` — wire 조립 전 비동기 훅. `rag`=`rag_search`(캔버스 '검색 결과'
  탭 + 발췌), `knowledge`=`list_knowledge`(16KB 상한); codebase의 경로 문구는 `pathContext`로 일반화됨
