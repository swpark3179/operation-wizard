...
```
wire = [ (첫 실제 작업 턴 1회) 내장 프롬프트 최적화 스킬(PROMPT_OPTIMIZER_SKILL — 레지스트리 밖,
           스텝 arming과 독립; 스킵 재귀·실패 시 unwindSkills로 되감김 — D78),
         (단계 armed면) step.skillIds의 스킬 body들(세션형은 대화당 1회;
...
```
홈 시작
  ├─ 카테고리 카드 클릭 → 그 카테고리로 진입(분류 없음)
  └─ 컴포저 프롬프트 전송 → 잠정 plan + autoCategory=true로 진입
        └─ (D81) WorkspaceView: ensureProject(cwd 확정) → classifyCategory(격리 턴, ```category)
                  → 최종 카테고리로 ChatPanel 1회 마운트 (실패/취소 → plan)
                  (분류 중: 채팅 컬럼 "작업 유형 분석 중…+중지" placeholder)
카테고리 진입 (위에서 정해진 카테고리)
  └─ optionsFor(category, settings) 비었나?
       ├─ 아니오 → onClarify([userRequest(text, 필수), codebasePath(folder, 필수)*, ...옵션]) [즉시 폼 · 채팅 차단]
       │            └─ seedPrompt 있으면 → userRequest 즉시 클라이언트 채움 + 프리필 턴(숨김, folder·noPrefill 제외)
       │                                    → onPrefill(자동채움 · seed 병합 — D78)
       │            └─ 사용자 폼 제출 → codebasePath 상태 분리 + answerSubmission(requirement 포함) [차단 해제 · 탭 소멸]
       │                  └─ 첫 작업 턴: 내장 프롬프트 최적화 스킬(D78) + WF[0].skills + WF[0].instruction + preflight + 답변(요구사항 포함)
       │                       ├─ end: ```prompt → '프롬프트' 탭(캔버스), 나머지 텍스트는 작업 계속
       │                       └─ (plan 기본, 기반 3단계 — D44)
...
**`guide`(운영 가이드 생성) 기본값(D63)**은 **부분 foundation**을 쓴다 — 코드베이스 단계를 빼고
`사내 문서 RAG 검색(rag)` → `지식 베이스 반영(knowledge)` → `운영 가이드 작성(document,
docs/operation-guide.md, output:html)` → chat. 이 카테고리의 강점은 **사내 문서(Confluence)를 시각적으로
보기 좋게** 보여주는 것으로, rag 단계 preflight가 캔버스 **'검색 결과' 탭**(카드 UI, D46)을 채우고,
`output:"html"`이 `html-render` 합성 서브스텝으로 `docs/operation-guide.html`을 자동 생성한다(D47). codebase가
없어 **필수 코드베이스 폴더 질문(D45)이 뜨지 않는다**(`optionsFor`가 resolved 워크플로우의 `codebase` 단계
유무로 게이팅). 스킬은 `guide-author`(RAG/지식 근거 인용·전제→단계→검증→롤백·프로세스 mermaid·참고
문서 섹션) + 렌더의 `html-render`. 옵션에 `referenceDocs`(참고 Confluence 공간/키워드)가 있다.
...
- **스킬 리소스 폴더(D45)**: 스킬 카드의 "리소스 폴더"로 참고 파일/스크립트 폴더(`SkillDef.dir`)를
  ...
  `RunArgs.extraDirs`(claude `--add-dir`, gemini `--include-directories` — D52; 원격 fabrix·aipro는
  파일 접근이 없어 무시 — D71)로 매 턴 접근 허용된다. 폴더 존재 검사는 없다(없으면 에이전트가 읽기 실패로
  표면화 — graceful degrade).
...
- 단계 규칙:
  ...
  - **기반 단계(D44)는 고정(pinned)**: 편집기에서 삭제·이동·kind 변경 불가(지시문·스킬·file은 편집
    가능). 카테고리가 pin하는 종류는 `CATEGORY_FOUNDATION`이 정한다 — **완전 트리오가 필수는 아니다**
    (`guide`는 코드베이스를 뺀 **rag+knowledge**만 — D63). `plan`은 항상 강제(`coerceSteps`가 프리펜드·
    보충), **`query`(D61)·`change`(D62)·`guide`(D63)는 기본값에 기반 단계를 포함**(`foundationEnabled`가
    stored 없으면 `DEFAULT_WORKFLOWS`로 폴백해 인식), 비-plan 카테고리는 "기반 단계 사용" 토글로 삽입/제거
    (저장 배열의 기반 kind 존재 = 활성 플래그; 토글은 카테고리 default의 기반 단계를 프리펜드). `stepsError`/
    `coerceSteps`의 순서 검사는 **canonical 부분수열**을 허용한다(`[rag,knowledge]` OK, `[rag,codebase]` 거부).
    rag/knowledge 단계는 지시문 아래에 **preflight 컨텍스트**(검색 발췌/지식 본문)가 자동 첨부되며, 미설정·
    0건이면 턴 없이 건너뛴다.
...
- 캔버스 렌더가 필요하면 `WorkspaceView`에 상태를 리프트하고 `CanvasPanel`에 탭/뷰를 추가한다(요구사항 폼이
  대표 예).
- **실전 예 — `src/lib/promptCraft.ts`(D78)**: 첫 작업 턴에 내장 스킬을 wire 최상단에 얹어 에이전트가
  ` ```prompt ` 펜스로 최적 프롬프트를 먼저 내게 하고, `parsePromptBlock`(태그 일치만·폴백 없음)/
  `stripPromptBlock`(→ `PROMPT_NOTE`)로 `end`에서 뽑아 캔버스 '프롬프트' 탭(`WorkspaceView.promptResult` +
  `CanvasPanel`의 `PromptPanel`)에 표시하는 최소 복제 사례다. 스텝이 아니라 `promptSkillPendingRef`로
  첫 실제 턴에 1회 주입하고 `unwindSkills`로 되감으므로, 스텝 파서(inflightStepRef)와 독립적으로 동작한다.
...
6. 자동 분류(⓪/D81)는 `CATEGORIES`(id·label·desc)를 그대로 프롬프트에 나열하므로, 새 카테고리는
   **별도 수정 없이 분류 후보에 포함**된다. 분류 판단 기준을 다듬으려면 `src/lib/categorize.ts`의
   `categorizeInstruction`(카테고리별 한 줄 판단 기준)만 수정한다.

### G. 카테고리 자동 분류(⓪) 동작 조정 (D81)
- 홈 컴포저 프롬프트로 시작할 때만 실행되는 격리 분류 턴. 지시문·파서·실행은 `src/lib/categorize.ts`
  (`categorizeInstruction`/`parseCategory`/`classifyCategory` — `ragRelevance.ts` 미러)에 있다.
- 오케스트레이션은 `src/components/WorkspaceView.tsx`(`classifying` state + `classifyStartedRef` 가드 effect):
  `ensureProject`로 cwd 확정 → `classifyCategory` → `setActiveCategory(best ?? "plan")` → `ChatPanel` 마운트.
  분류 중 placeholder(중지=`classifyCancelRef`)도 여기 있다.
- 신호 배선은 `HomeView`(`onStart`의 `autoCategory` 파라미터; 컴포저=true, 카드=false)와 `HomeArea`
  (`autoCategory` state → `WorkspaceView` prop). 분류 실패/취소/파싱실패는 항상 `plan` 폴백.
...
- **wire 조립**: `send()` — workdir 확정(`ensure_project`)을 먼저 수행한 뒤(D52)
  ...
  (claude `--add-dir`, gemini `--include-directories`; 원격 fabrix·aipro는 무시 — D71). `send()`는
  처리 여부를 boolean으로 반환하고 폼 제출/자동전진 nonce는 성공 시에만 소비된다(D55).
- **preflight**: `stepPreflight(step, cwd)` — wire 조립 전 비동기 훅. `rag`=`rag_search` 후
  **관련성 판단 턴**(D70, `judgeRagRelevance` — 격리 실행): 검색 결과가 이번 작업과 관련 없다고 판단되면
  건너뛰고(패널·발췌 주입 없음), 관련 있으면 판단 턴이 정리한 섹션을 캔버스 '검색 결과' 탭
  (`ragCuratedHtml`)에 표시 + 원본 발췌를 wire에 주입(판단 실패=`null`이면 fail-open으로 원본 패널·주입).
  `knowledge`=`list_knowledge`(16KB 상한; **artifact 엔트리(D59)는 요약+첨부 문서
  ...
  preflight 중 Stop은 `preflightAbortRef`로 취소 폴백(rag 판단 턴은 `ragJudgeCancelRef`로 함께 취소 — D70).
...
- **claude/codex(세션형)**: 스킬·단계 지시가 세션에 유지 → 가장 풍부(같은 스킬은 대화당 1회만 주입).
  프리필은 격리 세션으로 실행.
- **gemini(세션리스)**: 매 턴 transcript 재전송이라 과거 주입이 원문 그대로는 아니고 요약된 형태로만
  남는다(lossy). 단계별 주입이 반복되며 크기 증가. 코드베이스/스킬 폴더 접근은 `--include-directories`로
  부여된다(D52).
- **fabrix/aipro(원격 HTTP, 세션리스 — D64/D71)**: gemini처럼 매 턴 transcript 재전송(lossy)이나,
  **파일시스템·도구 접근이 없어** 코드베이스 실독·스킬 리소스 폴더 읽기가 불가하다(extraDirs 무시). 문서
  단계 산출물은 앱이 스트리밍 텍스트를 `write_file`로 저장한다(D67). 텍스트 스트림만(도구 이벤트 없음).
