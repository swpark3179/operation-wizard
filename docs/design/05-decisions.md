...
### D65. 요구사항 우선 필드 + 첫 작업 턴 '프롬프트 최적화' 내장 스킬 + '프롬프트' 캔버스 탭
- **배경**: 사용자 요구 두 가지 — ① 요구사항 폼(캔버스)에서 "뭘 하고 싶은지"가 우선되어야 한다
  (기존 자유 텍스트 질문은 카탈로그 맨 뒤였고, 카테고리 카드로 시작하면 요구사항을 적을 곳이 없었다).
  ② 폼 제출 후 답변들을 바탕으로 AI가 프롬프팅 기법을 활용해 최적 프롬프트를 완성해 캔버스에
  보여준다(사용자가 쓰면서 프롬프팅 개선법을 학습). 구현 제약(사용자 명시): 프로그램 구조를 바꾸지
  말고 **기존 스킬 주입 기능에서 모든 카테고리의 첫 단계에 스킬 하나를 더** 얹는 것.
- **결정(요구사항 필드)**: `options.ts`에 공용 `REQUIREMENT_QUESTION`(id `userRequest`, text, 필수)을
  추가하고 `optionsFor`가 **모든 카테고리 폼 맨 앞**에 프리펜드한다(codebase 질문보다 앞).
  홈 프롬프트(seed)로 시작하면 ChatPanel 부트가 **클라이언트 측에서 seed 원문으로 즉시 채우고**
  (`onPrefill`), 에이전트 프리필 완료 시에도 seed 값을 병합해 유지한다(`onPrefill`은 전체 교체형이라
  병합 없이는 나중 도착이 값을 지움). `ClarifyQuestion.noPrefill` 플래그를 신설해 이 질문을 에이전트
  프리필 대상에서 제외(folder 제외와 나란히 — 에이전트 재서술은 손실). 폼 제출 시 요구사항 답변은
  answers wire에 그대로 남고(최적 프롬프트 생성의 입력) `answerSubmission.requirement`로도 분리 전달되어,
  첫 작업 턴의 사용자 버블(display)이 되고 기존 "원래 요청:\n<seed>" 접미사를 대체한다(중복 제거).
- **결정(내장 스킬)**: 새 모듈 `lib/promptCraft.ts`의 `PROMPT_OPTIMIZER_SKILL`은 **레지스트리 밖
  하드코딩 내장 스킬**이다 — Flows 편집기/settings.json에 노출되지 않고 런타임에만 주입된다.
  근거: 스킬 레지스트리는 전체 교체형(D39)이라 `DEFAULT_SKILLS`에 넣으면 이미 override를 저장한
  사용자에게 적용되지 않는다(사용자 확정: 항상 적용). 주입 시점은 **단계가 아니라 대화의 첫 실제
  작업 턴**(`promptSkillPendingRef`, 로드 세션 제외) — guide처럼 첫 단계(rag)가 preflight에서 에이전트
  턴 없이 스킵되는 카테고리에서도 체인된 다음 턴에 실리도록, 되감기를 `unwindSkills`에 동승시켰다
  (preflight 중지/스킵/spawn 실패 시 플래그 복원). wire에서는 단계 스킬들보다 **앞**(최상단)에 놓인다.
- **결정(출력 계약·표시)**: 스킬은 에이전트에게 응답 **가장 처음에 ` ```prompt ` 펜스 하나**로
  최적 프롬프트(역할·상황·작업·제약·기대 산출물, 지어내기 금지)를 출력한 뒤 같은 턴에서 실제 작업을
  계속하라고 지시한다(별도 턴/확인 게이트 없음 — 사용자 확정 "같은 턴 인라인"). `end`에서
  `parsePromptBlock`(태그 일치만, **전체 응답 폴백 없음** — 폴백하면 작업 응답 전체를 프롬프트로 오인)로
  파싱해 `onPromptResult`로 리프트하고, 저장 content에서는 `stripPromptBlock`이 펜스를 안내문으로
  치환한다 — persist 전에 수행해 저장 세션·세션리스 transcript에 펜스가 남지 않는다(에이전트 재출력
  방지). 캔버스 **'프롬프트' 탭**은 rag '검색 결과' 탭(D46)과 동형: 결과 도착 시 자동 전환, 세션 동안
  유지, 새 세션/기록 열기 시 초기화. 본문은 안내줄 + 복사 버튼(`copyText`) + `<pre>` 평문(클라이언트
  보유 텍스트라 iframe 불필요).
- **대안 기각**: 별도 생성 턴 후 자동 진행/사용자 확인 게이트(턴 추가·마찰 — 사용자 기각) /
  `DEFAULT_SKILLS`+첫 단계 `skillIds` 등록(override 사용자 미적용, rag 스킵 시 유실) /
  `runtimeWorkflowFor` 변환으로 step[0]에 부착(동일한 rag 스킵 문제).
- **한계**: plain(opencode/antigravity)·펜스 미준수 에이전트는 파싱 실패 → 탭 없음, 작업은 그대로
  (우아한 폴백, D30 관례). 스트리밍 중에는 펜스 원문이 잠시 보이다가 완료 시 치환(D57 평문 관례와
  동일). 스트림 도중 실패한 턴의 재시도는 스킬 dedupe와 동일하게 재주입하지 않는다(wire가 이미
  에이전트에 도달) — 재시도 턴이 펜스를 다시 내면 일반 코드블록으로 보일 수 있다(수용).
- **백엔드 무변경, 신규 의존성 0** (전부 프론트).
