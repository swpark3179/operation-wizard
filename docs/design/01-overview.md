...
- 디자인의 아티팩트별 전용 캔버스 탭(저장소 분석/영향도/변경 가이드 등) — `plan` 계획서 문서는 파일
  뷰어로 표시되지만 전용 뷰/오케스트레이션은 후속. (요구사항 명확화·소스 조사·계획서 생성은 `plan`에 한해
  증분 3→4의 카테고리 워크플로우로 구현됨. plan 외 카테고리의 실제 다단계 플로우는 후속.)
