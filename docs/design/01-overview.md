...
- **카테고리 가이드 플로우(증분 3→6):** 카테고리마다 **고정 선택지 우선 시작 + 단계 오케스트레이터(단계별
  ...
  진입 시 프롬프트가 아니라 **카테고리별 고정 선택지 폼을 먼저** 보여주고(폼 맨 앞엔 필수 **요구사항
  자유 입력 필드** — 홈 프롬프트로 시작했으면 그 원문으로, 나머지 항목은 숨김 프리필 턴으로 자동 채움;
  **폼 대기 중 채팅 차단, 제출 시 '요구사항' 탭 소멸**), 폼 제출이 첫 작업
  턴을 발사한다. 첫 작업 턴에는 **프롬프트 최적화 내장 스킬**이 항상 주입되어 답변 기반 최적 프롬프트를
  캔버스 **'프롬프트' 탭**으로 보여준다(교육적 표시 — D65). 프롬프트 주입 + 텍스트 블록/도구 이벤트
