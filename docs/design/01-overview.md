...
- **기반 3단계 + 지식 인프라(증분 7):** 워크플로우 앞에 **반드시 거치는 기반 3단계**를 도입했다 —
  ...
  `plan`·`query`(D61)·`change`(D62)는 항상 강제(기본값에 기반 3단계 포함), guide는 Flows 토글로 opt-in.
...
- 디자인의 아티팩트별 전용 캔버스 탭(저장소 분석/영향도/변경 가이드 등) — **집계 뷰는 캔버스 '산출물'
  ...
  변경 종류별 DC Manager 신청양식(HTML) 생성은 `change`(D62)에 한해 카테고리 워크플로우로 구현됨.
  guide의 실제 다단계 플로우는 후속.)
