...
- **extraDirs 에이전트별 지원**: **claude** = entry마다 `--add-dir`(호출 단위라 매 턴 전달) /
  **codex** = full-access 샌드박스라 불필요 / **gemini·aipro·plain** = 무시(프론트가 경로를 항상
  프롬프트에도 언급하므로 best-effort로 degrade).
...
- **WORKSPACE**(`WorkspaceView`): 좌 `ChatPanel`(412px) + 우 `CanvasPanel`. `WorkspaceView`가
  **세션 remount 키(`sessionNonce`) + `loadedSession` + `resolvedWorkdir`**를 소유한다 — 새 세션/기록
  ...
  - `CanvasPanel`: 툴바에 **요구사항 / 검색 결과 / 파일** 탭 토글 + 파일 트리(`listDir`, 지연 확장,
    `refreshNonce`로 문서 생성 후 리로드 — `FileViewer`에도 전달해 열린 파일 재읽기)·`FileViewer`, 또는
    요구사항 폼(`RequirementsForm`, **accent 카드 그리드** — [05](05-decisions.md) D35). **'요구사항' 탭
    pill은 폼이 사용자 답변을 기다리는 동안만 렌더**되고, 제출/초기화로 `clarify`가 비면 탭이
    사라진다(`effectiveTab` 파생으로 pill 없는 상태 방어 — D41). **'검색 결과' 탭**은 rag 기반 단계가
...
- **①.5 기반 3단계(foundation, D44)** — 워크플로우 맨 앞에 고정(pinned)되는 필수 단계:
  `codebase`(코드베이스 분석 — preflight가 "분석 대상 코드베이스 폴더: <path>"를 주입하고 매 턴
  `extraDirs`로 접근 부여) → `rag`(preflight가 `rag_search`로 사내 문서 발췌를 조회해 wire에 첨부 +
...
- **커서(ChatPanel)**: `stepIndexRef`(현재 단계) + `stepArmedRef`(단계당 1회 주입) +
  ...
  대화(재주입 없음). wire 조립은 `[step.skillIds의 스킬 body들(armed, 세션형 dedupe; dir 있으면 리소스
  폴더 안내 부착) → step.instruction(armed) → preflight 컨텍스트(기반 단계) → prompt]`.
- **`end` 분기**:
  ...
  - 생성형(`isGenerative` — search/document/기반 3종): `file`이 있으면 `onOpenFile(join(cwd,file))`로
    캔버스에 연다 → 다음 단계가 생성형이면 **자동 진행**(`setAutoTurn` nonce+effect; stale 클로저 회피),
    종단 `chat`이면 멈춤. 검색 근거는 스트리밍되는 `toolUse`/`toolResult`(에이전트 Grep/Read)로 표시된다.
...
- **리프트(WorkspaceView)**: `clarify`(옵션 질문)/`clarifyPrefill`(자동채움 답변)/`prefillNonce`/`canvasTab`/
  ...
  `codebasePath`(프로젝트 스코프 — 새 세션에도 유지) + `ragResult`(검색 결과 탭 HTML) 소유.
