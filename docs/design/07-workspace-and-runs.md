...
- **fabrix**(원격 HTTP API, `kind: Remote` — D64): `RunSpec`을 쓰지 않는다(`run: None`). `run_agent`가
  최상단에서 `fabrix::run_fabrix`로 위임 — 프로세스 spawn/stdin 없이 `POST {endpoint}/openapi/chat/v1/messages`
  (헤더 `x-fabrix-client`/`x-openapi-token`, body `{modelIds:[model], contents:[prompt], llmConfig, isStream:true,
  systemPrompt}`)를 보내고 **SSE 응답**을 파싱해 `RunEvent`로 방출한다. `extra_dirs`/`cwd`는 무시(파일시스템
  무관). 세션리스(gemini/aipro처럼 매 턴 transcript 재전송). SSE 파서 `parse_fabrix_sse_data`
...
느린 I/O는 IPC 스레드를 건드리지 않는다. (**`kind==Remote`(fabrix)면 최상단에서 `fabrix::run_fabrix`로
위임** — 아래 5. 원격 경로 참조, D64.)
...
4. `cancel_run(runId)`: `canceled` 플래그 set + **프로세스 트리 종료**. Windows에선 `.cmd` shim이
   ...
   `Option`이라 **원격(fabrix) 런은 플래그만 set**하고 taskkill을 건너뛴다(SSE 루프가 플래그를 관측해 연결 종료).
5. **원격 경로(fabrix, D64)**: `run_fabrix`가 config 로드 → `RunRegistry`에서 id 민팅(`next_id`) +
   자식 없는 취소 플래그 등록(`register_remote`) → 워커 스레드에서 `POST .../messages` → 응답을
   `BufReader`로 읽어 `data:` 라인을 lossy 디코드(`run::stream_lines`와 동일 방식) → `parse_fabrix_sse_data`로
   `RunEvent` 방출(루프 사이 취소 플래그 확인) → 스트림 종료 후 `unregister` + 단일 `end`(실패면 `Error` 선행).

> **취소 지연(fabrix)**: 취소 플래그는 SSE `data:` 라인 사이에서 확인되므로 서버가 조용하면 다음 바이트까지
...
- **fabrix — 세션리스**(D64): API에 세션 개념이 없어 gemini/aipro와 동일하게 transcript를 매 턴
  `contents:[transcript]`로 재전송한다. ChatPanel `SESSION_AGENTS`에 미포함 → 자동으로 세션리스 경로.
...
| 커맨드 | 입력 | 출력 | 비고 |
|--------|------|------|------|
| `run_agent` | `args: RunArgs`, `onEvent: Channel<RunEvent>` | `runId: string` | 워커 스레드에서 스트리밍. `RunArgs {agentId, prompt, cwd, model?, sessionId?, resume?, extraDirs?}`. `kind==Remote`(fabrix)면 `fabrix::run_fabrix`로 위임(POST+SSE — D64) |
| `cancel_run` | `runId: string` | — | 프로세스 트리 종료(`taskkill /T /F` + `child.kill()`) → `end{status:"canceled"}`. 원격(fabrix)은 자식 없이 취소 플래그만(SSE 루프가 연결 종료 — D64) |
...
| `set_fabrix_config` / `probe_fabrix` | `config \| null` / — | `Settings` / `string` | Agents 뷰 Fabrix 카드: 연결 설정 저장·해제(빈 endpointUrl=해제) / 연결 테스트("연결됨 (N개 모델)") — D64 |
