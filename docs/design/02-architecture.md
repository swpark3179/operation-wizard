...
| 가이드 플로우 | `lib/options.ts`, `lib/skills.ts`, `lib/workflow.ts` | 카테고리별 고정 선택지 카탈로그(`CATEGORY_OPTIONS`, 코드 정적) + **settings-aware 스킬 레지스트리(`DEFAULT_SKILLS`/`resolveSkills`)·워크플로우(`DEFAULT_WORKFLOWS`/`workflowFor(category, settings)`)** — 사용자 override는 settings.json, 코드 기본값은 폴백=샘플. `plan`·`query`(D61)·`change`(D62)는 다단계 기본값(기반 3단계 포함), `change`는 종류별 DC Manager 신청양식(HTML) 산출 ([08](08-guided-flows-and-skills.md), D39/D40/D61/D62) |
