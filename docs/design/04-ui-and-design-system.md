...
- `HomeView`: 히어로(운영 도구 프레이밍 — 배지 "Samsung SDS · Operation Wizard" + "운영 작업 도우미" +
  ...
  `set_project_title` — D60). 카테고리/전송 → **새 프로젝트** + 워크스페이스 진입.
...
- `WorkspaceView`: 좌 `ChatPanel`(기본 412px, **경계 드래그로 폭 조절·localStorage 기억** — D49;
  ...
  서식 붙여넣기 — D62); **'요구사항' 탭은 폼이
  대기 중일 때만 표시**(폼 맨 앞엔 필수 **요구사항 자유 입력 필드** — D65); **'프롬프트' 탭** = 첫
  작업 턴에 주입된 프롬프트 최적화 내장 스킬이 낸 최적 프롬프트 표시(안내줄+복사 버튼+평문 `<pre>`,
  결과 도착 시 자동 전환·세션 동안 유지 — D65); **'산출물' 탭** = 워크플로우 산출 문서 목록(단계 상태 칩)+미리보기 허브,
  **'다이어그램' 탭** = 산출물 mermaid 갤러리(+**창 전체 확대 모달**: 확대/축소 컨트롤 0.25×~4×,
  ...
  (편집 가능)을 지식 베이스에 저장). 파일 탭 툴바의 폴더 칩은 **실제 절대경로**(좌측 말줄임 + 툴팁)를
  표시한다(D60). 상세 동작은
