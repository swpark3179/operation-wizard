...
- `HomeView`: 히어로(운영 도구 프레이밍 — 배지 "Samsung SDS · Operation Wizard" + "운영 작업 마법사" +
  진행 절차 안내, D60) + 프롬프트 컴포저(**에이전트·모델 셀렉트 포함** — 시작 전에 실행 조합을 고르고
  ChatPanel 초기 선택으로 시드됨, D60) + **작업 폴더 지정 버튼**(컴포저 바로 아래;
  미지정 시 새 프로젝트 폴더 자동 생성) + **4개 업무 카테고리**(개발 계획 수립 / 운영 가이드 생성 / 데이터
  조회 / 데이터 변경·권한) + 최근 작업(프로젝트 목록 — 행 hover 연필 버튼으로 **제목 인라인 변경**,
  `set_project_title` — D60). 카테고리/전송 → **새 프로젝트** + 워크스페이스 진입. **컴포저 프롬프트로
  시작하면 카테고리를 고정하지 않고 진입 후 프롬프트에 가장 맞는 카테고리로 자동 분류하며(컴포저 라벨
  "작업 유형 자동 선택"), 카테고리 카드 클릭은 그 카테고리로 명시 진입한다(D81).**
  진입 시(자동 분류 완료 후) 프롬프트 대화가 아니라 **카테고리별 고정 선택지 폼을 먼저** 보여주고
  (프롬프트로 시작했으면 아는 값 자동 채움), 폼 제출이 첫 작업 턴을 발사한다([07](07-workspace-and-runs.md)·[08](08-guided-flows-and-skills.md) D36/D81).
- `WorkspaceView`: 좌 `ChatPanel`(기본 412px, **경계 드래그로 폭 조절·localStorage 기억** — D49;
  ...
  마크다운+mermaid 미리보기(D42) + **목차 드롭다운**(h1~h3 섹션 점프 — D58); `.html`은 sandbox iframe
  미리보기 + 파일바 **"본문 복사" 버튼**(문서 `<body>`를 `text/html`로 복사 → DC Manager 등 편집기에
  서식 붙여넣기 — D62); **미리보기 HTML·마크다운의 외부 링크(http/https/mailto)는 클릭 시 앱 뷰를
  이동시키지 않고 OS 기본 브라우저로 열린다**(운영 가이드 참고 문서 링크가 앱을 브라우저로 만들던 문제
  해소 — D76); **'요구사항' 탭은 폼이
  대기 중일 때만 표시**; **'산출물' 탭** = 워크플로우 산출 문서 목록(단계 상태 칩)+미리보기 허브,
  **'다이어그램' 탭** = 산출물 mermaid 갤러리(+**창 전체 확대 모달**: 확대/축소 컨트롤 0.25×~4×,
  CSS `zoom` 배율 — D60) — 둘 다 산출물이 있는 워크플로우에서만 표시,
  워크플로우 산출 파일은 파일 탭 대신 산출물 탭으로 열림(D58); **'지식 저장' 탭**(`KnowledgeSavePanel`,
  D59) = 워크플로우 완료 배너/산출물 행 액션으로 열리는 조건부 탭 — 산출물 체크박스+제목+에이전트 요약
  (편집 가능)을 지식 베이스에 저장); **'검색 결과' 탭** = rag 단계의 사내 문서 검색 결과가 **관련성 LLM
  판단(D70)을 통과했을 때만** 나타나는 정리된 뷰(sandbox iframe srcdoc; 관련 없다고 판단되면 미표시,
  D46/D70)); **'프롬프트' 탭**(D78) = 폼 제출 후 첫 작업 턴에 에이전트가 구성한 **최적 프롬프트**를
  보여주는 인메모리 평문 뷰(`PromptPanel`, 복사 버튼 — 도착 시 자동 표시·세션 유지; 프롬프팅 학습용).
  파일 탭 툴바의 폴더 칩은 **실제 절대경로**(좌측 말줄임 + 툴팁)를
  표시하고(D60), 칩만 축소 허용(`min-w-0`)해 긴 경로가 좌측 버튼을 짓누르지 않으며, 칩 오른쪽의
  **"탐색기에서 열기"** 버튼이 현재 트리 루트를 OS 탐색기에 연다(D69). 상세 동작은
...
- 사용성 패키지 2차(D60): 캔버스 뷰어 **flex 높이 제약(`min-h-0`) 수정**으로 파일 내용 스크롤·목차
  팝오버 위치 정상화, 파일바 **미리보기/소스 토글 shrink 고정**(경로만 말줄임), 스트리밍 중
  **라이브니스 상태줄**(경과 시간 + 마지막 응답 시각 + 장기 무응답 안내).
...
- `KnowledgeView`(D48): ① **RAG 검색 설정** — ENDPOINT_URL / x-fabrix-client / x-openapi-token(요청 헤더 값,
  D50/D65) / Knowledge Asset ID(비우면 샘플 자산) / Top K + 연결 테스트(`probe_rag` = rag-chat `/models`
  조회, 프록시 우회) + **캐시된 모델 목록(접이식, `cached` 배지)** — 저장·연결 테스트가 조회해
  `rag.models`에 저장하고 이후 무네트워크 표시(캐시 우선, D66; 실제 검색은 GLM 5.2 고정), ② **Confluence 수집(MCP)** — 연결 설정(MCP URL(프리필)+x-auth 키) + 연결 테스트(`probe_confluence`=initialize+tools/list) + 수집 대상(루트 페이지 ID/검색어) + 수집 시작·중지 + 진행 표시(수집/저장/실패 카운트·티커); 수집 문서는 지식 베이스 artifact로 저장(D82), ③ **지식 베이스** CRUD 카드
  (저장된 항목 삭제는 확인 다이얼로그 — D57; RAG 오류는 사용자용 문구로 표시).
  **산출물 지식(D59)**은 카드에 accent '산출물' 배지 + 출처 라인(카테고리·원요청) + 읽기 전용 첨부
  문서 목록을 표시한다(body=요약은 편집 가능; 삭제 확인 문구에 첨부 문서 동반 삭제 안내).
