...
`AGENT_DEFS`는 6개 에이전트 정의를 **표시 순서대로** 담은 정적 배열이다.
`find(id)`로 조회하고, `all()`로 전체를 얻는다. 값은 Open Design defs에서 1:1 포팅했다.

| id | name | bin 후보 | env override | 추가 검색 하위경로 | 모델 명령 / 파서 / 타임아웃 | fallback 모델 (default는 코드에서 prepend) |
|----|------|----------|--------------|-------------------|------------------------------|---------------------------------------------|
| `opencode` | OpenCode | `opencode-cli`, `opencode` | `OPENCODE_BIN` | `.opencode\bin` | `models` / line / 15s | `anthropic/claude-sonnet-4-5`, `openai/gpt-5`, `google/gemini-2.5-pro` |
| `claude` | Claude Code | `claude`, `openclaude` | `CLAUDE_BIN` | — | (없음 → fallback) | `sonnet`(Sonnet alias), `opus`, `haiku`, `claude-opus-4-5`, `claude-sonnet-4-5`, `claude-haiku-4-5` |
| `codex` | Codex CLI | `codex` | `CODEX_BIN` | — | `debug models` / JSON / 5s | `gpt-5.5`, `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.3-codex`, `gpt-5.1`, `gpt-5.1-codex-mini`, `gpt-5-codex`, `gpt-5`, `o3`, `o4-mini` |
| `gemini` | Gemini CLI | `gemini` | `GEMINI_BIN` | — | (없음 → fallback) | `gemini-3-pro-preview`, `gemini-3-flash-preview`, `gemini-2.5-pro`, `gemini-2.5-flash`, `gemini-2.5-flash-lite` |
| `antigravity` | Antigravity | `agy` | `ANTIGRAVITY_BIN` ※ | — | (없음 → fallback) | `Gemini 3.1 Pro (High/Low)`, `Gemini 3.5 Flash (High/Medium/Low)`, `Claude Sonnet 4.6 (Thinking)`, `Claude Opus 4.6 (Thinking)`, `GPT-OSS 120B (Medium)` |
| `aipro` | AI Pro | `aipro` | `AIPRO_BIN` ✦ | — | (없음 → fallback) | `glm-5.1`(GLM-5.1), `qwen3.6-27b`(Qwen3.6-27b), `gpt-oss-120b`(Gpt-Oss-120b) |
...
`AgentDef` 필드: `id`, `name`, `bin_candidates`, `env_var: Option`, `extra_search_subdirs`,
`version_timeout`, `models_probe: Option<ModelsProbe>`, `fallback_models: &[(id,label)]`,
`run: Option<RunSpec>`(실행 스펙 — 탐지가 아닌 **실행**용, 자세히는 [07-workspace-and-runs.md](07-workspace-and-runs.md)).
...
`detect_agent_blocking(def, custom)`가 한 에이전트를 탐지한다. version args는 5개 모두
`--version`이라 하드코딩하고, 타임아웃·모델 명령·파서·fallback은 모두 def에서 가져온다.
...
| `source` | `custom-path` / `path` / `not-found` |
| `models` | `{ id, label }[]` (항상 맨 앞 `default`) |
| `modelsSource` | `live` / `fallback` |
| `diagnostic` | `not-on-path` / `not-executable` / `missing-target` / null |

프론트의 `DIAGNOSTIC_HINT` 맵이 진단 코드를 사용자용 안내문으로 변환한다.

## 설정 영구화 (`settings.rs`)

- `Settings { agents: { [agentId]: { customBin: string | null } }, skills?, workflows? }` —
  에이전트별 경로 맵 + **워크플로우 단계/스킬 override**(탐지와 무관, [07](07-workspace-and-runs.md)·
  [05](05-decisions.md) D39).
...
- **codex JSON 파서 단위 테스트**: hidden 스킵 / slug·id / display_name·name / 중복 제거 /
  빈·비JSON → `None`.
- **레지스트리 sanity**: 6개 id 유일·비어있지 않음, fallback 비어있지 않음, 6개 id 조회됨.
...
1. `agents.rs`의 `AGENT_DEFS`에 `AgentDef` 항목 1개 추가(bin 후보/env/타임아웃/fallback).
